...
let play_tick () =
  let () = Lwt_preemptive.set_bounds(!beats_played+1,!beats_played+1) in
  Lwt.async (fun () -> 
    Lwt_preemptive.detach (fun () ->
      (let _ = Sys.command "afplay ./wav_files/metronome-trimmed.wav" in () )
    ) () >>= fun _ -> 
    Lwt.return_unit
  )
...
let rec start_metronome start_time () = 
  if Atomic.get is_on then
    ...
    if bpm > 0. then 
      ...
      while Unix.gettimeofday () < start_time do () done; (* Loop until Unix.gettimeofday () is start_time *)
      play_tick ();
      beats_played := !beats_played + 1;
      let next_time = Unix.gettimeofday () +. interval in (* Capture end time after playing tick *)    
