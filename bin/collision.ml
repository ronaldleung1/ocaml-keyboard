...
let rec loop (pause, box_a, box_a_speed_x, box_b, box_collision) =
  ...
  | false ->
      ...
      (if not pause then
         Rectangle.(set_x box_a (x box_a +. box_a_speed_x)));
      ...
      Rectangle.(
        set_x box_b
          (Float.of_int (get_mouse_x ()) -. (width box_b /. 2.0)));
      Rectangle.(
        set_y box_b
          (Float.of_int (get_mouse_y ()) -. (height box_b /. 2.0)));
      ...
      (if
       ...
       then
         Rectangle.(
           set_x box_b
             (Float.of_int (get_screen_width ()) -. width box_b))
       else if Rectangle.(x box_b <= 0.0) then
         Rectangle.(set_x box_b 0.0));
      (if
         Rectangle.(
           y box_b +. height box_b
           >= Float.of_int (get_screen_height ()))
       then
         Rectangle.(
           set_y box_b
             (Float.of_int (get_screen_height ()) -. height box_b))
      ...
      let box_collision =
        if collision then get_collision_rec box_a box_b
        else box_collision
      in

      (* Pause Box A movement *)
      let pause =
        if is_key_pressed Key.Space then not pause else pause
      in
