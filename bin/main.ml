...
let rec loop
  ...
  if Raylib.window_should_close () then 
    ...
    (* let () = print_string_to_file "saved.txt" (tuple_to_string (metronome.bpm, !volume_slider, !current_instrument)) in *)
    Raylib.close_window ()
  else
    let open Raylib in
    begin_drawing ();
    ...
    let selected_instrument =
      List.nth instruments !list_view_active |> fst in 
      let () = print_string selected_instrument 
    in
...
let () = print_string_to_file ("saved.txt") (array_to_string !saved)
  ...
  let rec loop
    ...
    if Raylib.window_should_close () then 
      ...
      (* let () = print_string_to_file "saved.txt" (tuple_to_string (, !volume_slider, !current_instrument)) in  *)
      Raylib.close_window ()
    else
      let open Raylib in
      begin_drawing ();
      ...
      let selected_instrument =
        ...
        (* Fallback to the current instrument if the index is out of
           bounds *)
      in
      (* curr := selected_instrument;
      let () = print_endline ("hello hello hello" ^ !curr) in *)
      if
      ...
      then begin
        ...
        in
        end_drawing ();
        loop metronome keys octave_keys volume_control current_instrument
      end
      else end_drawing ();
      loop metronome keys octave_keys volume_control current_instrument
  ...
  let () =
    if List.mem "playlist" argv then library_menu Library.empty
      ...
      let metronome, keys, octave_keys, volume_control = setup () in
      loop metronome keys octave_keys volume_control (ref "piano")
    end